/**
 * Seed
 * Copyright (C) 2021 EUU⛰ROCKS
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */
package org.seed.ui.zk.vm.admin;

import java.io.Serializable;

import org.seed.core.form.layout.Alignment;
import org.seed.core.form.layout.Orientation;

public class LayoutDialogPreferences implements Serializable {

	private static final long serialVersionUID = -9120094827215269245L;
	
	private Orientation labelOrientation;
	
	private Alignment labelAlign;
	
	private Alignment labelValign;

	public Orientation getLabelOrientation() {
		return labelOrientation;
	}

	public void setLabelOrientation(Orientation labelOrientation) {
		this.labelOrientation = labelOrientation;
	}

	public Alignment getLabelAlign() {
		return labelAlign;
	}

	public void setLabelAlign(Alignment labelAlign) {
		this.labelAlign = labelAlign;
	}

	public Alignment getLabelValign() {
		return labelValign;
	}

	public void setLabelValign(Alignment labelValign) {
		this.labelValign = labelValign;
	}
	
}
